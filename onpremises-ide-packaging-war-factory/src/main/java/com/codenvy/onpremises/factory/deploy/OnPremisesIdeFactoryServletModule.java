...
import com.codenvy.onpremises.factory.OnPremisesIdeFactoryServlet;
import com.codenvy.onpremises.factory.filter.AgreementPageFilter;
import com.codenvy.onpremises.factory.filter.BrowserCheckerFilter;
import com.codenvy.onpremises.factory.filter.ContributionFlowFilter;
...
@DynaModule
public class OnPremisesIdeFactoryServletModule extends ServletModule {

    private static final String PASS_RESOURCES_REGEXP = "^(?!/resources/)/?.*$";

    @Override
    protected void configureServlets() {
        filterRegex(PASS_RESOURCES_REGEXP).through(BrowserCheckerFilter.class);
        filterRegex(PASS_RESOURCES_REGEXP).through(FactoryParamsFilter.class);
        ...
        filterRegex(PASS_RESOURCES_REGEXP).through(AgreementPageFilter.class);
        filterRegex(PASS_RESOURCES_REGEXP).through(ReferrerCheckerFilter.class);
        filterRegex(PASS_RESOURCES_REGEXP).through(ContributionFlowFilter.class);
        ...
        serveRegex(PASS_RESOURCES_REGEXP).with(OnPremisesIdeFactoryServlet.class);
