...
import com.codenvy.auth.sso.client.SessionStore;
...
/**
 * Fork of the LoginFilter
 * Difference is that it redirects to the provided login page if auth is not done.
 *
...
public class AdminFilter implements Filter {
        ...
        if ("GET".equals(method) && httpReq.getParameter("login") != null) {
            ...
            if (token != null) {
                ...
                if (principal == null || !principal.hasUserInContext(rolesContext)) {
                    handleRedirectLoginPage(httpReq, httpResp, clientUrl);
                    return;
                }
    ...
    private void handleRedirectLoginPage( HttpServletRequest httpReq, HttpServletResponse httpResp, String clientUrl) throws IOException {
        UriBuilder redirectUrlParameter =
                UriBuilder.fromUri(httpReq.getRequestURL().toString()).replaceQuery(httpReq.getQueryString());
        String redirectUrl = UriBuilder.fromUri(loginPageUrl)
                                       .queryParam("redirect_url",
